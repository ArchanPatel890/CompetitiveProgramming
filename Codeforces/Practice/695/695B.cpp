...
int isValley(int i, vi &a, int n) {
    return (i > 0 && i < n - 1 && a[i] < a[i - 1] && a[i] < a[i + 1]);
}

int isHill(int i, vi &a, int n) {
    return (i > 0 && i < n - 1 && a[i] > a[i - 1] && a[i] > a[i + 1]);
}
...
void solve(vi &a, int n) {
    ...
    int s = 0;
    for (int i = 1; i < n - 1; i++) {
        if (isHill(i, a, n) || isValley(i, a, n))
            is[i] = 1, s++;
    }
    ...
    for (int i = 1; i < n - 1; i++) {
        int temp = a[i];
        a[i] = a[i - 1];
        ans = min(ans, s - is[i - 1] - is[i] - is[i + 1] 
		+ isHill(i - 1, a, n) + isValley(i - 1, a, n) 
		+ isHill(i, a, n) + isValley(i, a, n) 
		+ isHill(i + 1, a, n) + isValley(i + 1, a, n));
        a[i] = a[i + 1];
        ans = min(ans, s - is[i - 1] - is[i] - is[i + 1] 
		+ isHill(i - 1, a, n) + isValley(i - 1, a, n) 
