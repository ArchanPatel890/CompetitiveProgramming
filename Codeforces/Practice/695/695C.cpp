...
void solve(vll a1, int n1, vll a2, int n2, vll a3, int n3) {
	insertMin(a1);
	insertMin(a2);
	insertMin(a3);
	vvll av = {a1, a2, a3};
	sort(all(av), [](auto a, auto b) { return a[0] < b[0]; });
	vll three = {av[0][0], av[1][0], av[2][0]};
	vll sums;
	...
	ll ans = INT64_MIN;
	for (int i = 0; i < 3; ++i) {
		ll one = three[i%3]-sums[(i+1)%3]-sums[(i+2)%3];
		ll two = three[min((i+1)%3, (i+2)%3)]-sums[i];
		ll thr = three[max((i+1)%3, (i+2)%3)];
		vll a = {one, two, thr};
		sort(all(a));
		ll tmp = 0;
		if (a[0] > 0 || a[1] > 0) {
