...
void solve(vpii &rooks, int m, int n) {
	vi next(n+1, 0);
	for (auto r : rooks) {
		next[r.ft] = r.sc;
	}
	...
	vi seen(n+1, 0);
	function<bool(int,int*)> GetCycle = 
	[&](int node, int* len) {
		if (seen[node] == 1) return true;
		if (seen[node] == 2) return false;
		if (next[node] == 0) return false;
		seen[node] = 1;
		(*len)++;
		bool ok = GetCycle(next[node], len);
		if (ok) return ok;
		else {
			seen[node] = 2;
			return ok;
	...
	for (auto r : rooks) {
		...
		int len = 0;
		bool isCycle = GetCycle(r.ft, &len);
		total += len;
		if (isCycle && len > 1) total++;
	}
	cout << total << endl;
}
