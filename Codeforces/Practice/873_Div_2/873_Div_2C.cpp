...
int binary_search(function<bool(int)> f, int from_inclusive, int to_inclusive) {
...
void solve()
{
  ...
  cin >> n;
  vi a(n), b(n);
  for (int& ai : a) ai = readInt<int>();
  for (int& bi : b) bi = readInt<int>();
  ...
  for (int i = 0; i < n; ++i) {
    ...
    int pos = binary_search([&](int x) { return b[x] >= target; }, i, n-1);
