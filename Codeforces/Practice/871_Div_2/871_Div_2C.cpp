...
void solve(ll n, vector<pair<ll, string>>& books)
{
  ll min_11 = INF, min_01 = INF, min_10 = INF;
  for (auto& [m, skills] : books) {
    debug(m);
    if (skills == "11") {
      amin(min_11, m);
    }
  ...
  ll min_time = INF;
  amin(min_time, min_11);
  amin(min_time, min_01 + min_10);
...
int main()
{
  ...
  while (tc--)
  {
    ll n;
    cin >> n;
    vector<pair<ll, string>> books(n);
    for (auto& [m, skill] : books) {
      cin >> m;
      cin >> skill;
    }
    solve(n, books);
