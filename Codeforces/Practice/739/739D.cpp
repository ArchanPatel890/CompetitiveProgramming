...
void solve(ll n, vector<string>& seq) {
	string str = to_string(n);
	int len = str.size();
	int ans = INT_MAX;
	for (string pow : seq) {
		int pow_len = pow.size();
		int ptr = 0;
		int i = 0;
		while (i < len && ptr < pow_len) {
			if (str[i] == pow[ptr]) {
				++ptr;
			}
			++i;
		...
		amin(ans, pow_len - ptr + len - ptr);
	}

	cout << ans << endl;
}

vector<string> gen_pow_2(ll n) {
	vector<string> seq;
	ll pow = 1;
	while (pow < n) {
		seq.push_back(to_string(pow));
		pow *= 2;
	}
	return seq;
}
