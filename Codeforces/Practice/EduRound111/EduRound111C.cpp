...
void solve(vi a, int n) {
	...
	while (end < n) {
		...
		int start = end;
		while (start >= 0) {
			auto iptr = upper_bound(all(lis), a[start]);
			if (iptr != lis.end()) {
				(*iptr) = a[start];
			}
			else {
				lis.push_back(a[start]);
			}

			auto dptr = upper_bound(all(lds), a[start], std::greater<int>());
			if (dptr != lds.end()) {
				(*dptr) = a[start];
			}
			else {
				lds.push_back(a[start]);
			}
			if (lis.size() >= 3 || lds.size() >= 3) break;
			--start;
		}
		ans += (end - start);
		++end;
